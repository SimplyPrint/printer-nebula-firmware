...
function write_ota_info() {
    ...
    {
        echo "ota_board_name=${board_name}"
        echo "ota_compile_time=$(date '+%Y %m.%d %H:%M:%S')"
        echo "ota_site=http://192.168.43.52/ota/board_test"
    } >> "/tmp/${version}-simplyprint/ota_info"
...
part_md5=
for i in "/tmp/${version}-simplyprint/$directory/$sub_directory"/rootfs.squashfs.*; do
    [[ -e "$i" ]] || break
    file=$(basename "$i")
